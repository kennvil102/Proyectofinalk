...
const Register = () => {
  ...
  // Funcion de validación  si la funcion no esta vacia se prosigue con la funcion registro
  const validarCampos = () => {
    if (!email.trim() || !password.trim()) {
      alert("Error: Los campos no pueden estar vacíos o contener solo espacios en blanco");
      return false;
    }
    return true;
  };
  ...
  const registro = () => {
    if (validarCampos()) {
      // Solo registrar si valida  que no hay campos vacios
      async function datos() {
        // Función asíncrona para enviar los datos de registro
        let regis = await postData(email, password); // Llama a la función postData para registrar el correo y la contraseña
    ...
    <div>
      <div className="movereldi">
        <div className="moverR">
          ...
          {/* inputs que captura los datos de las celdas por medio del evento onchange*/}
